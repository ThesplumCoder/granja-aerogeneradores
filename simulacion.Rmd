...
while(length(failures_times) < failures_amount) {
  fail <- random_exponential(trials = 1, lambda = 1/72)
  if (fail < hours) {
    if (length(failures_times) == 0) {
      failures_times <- c(failures_times, fail)
      repairs_times <- c(repairs_times, fail + 8)
    }
    if (fail > tail(repairs_times, n = 1)) {
      failures_times <- c(failures_times, fail)
      repairs_times <- c(repairs_times, fail + 8)
    }
  }
...
plot(x = failures_times, 
     y = 1:length(failures_times), 
     xlim = c(0, hours),
     col = "red",
     main = "Visualización de los pares falla-reparación",
     xlab = "Hora de la semana",
     ylab = "Número del par")
points(x = sort(failures_times) + 8, 
       y = 1:length(failures_times),
       col = "blue")
```
